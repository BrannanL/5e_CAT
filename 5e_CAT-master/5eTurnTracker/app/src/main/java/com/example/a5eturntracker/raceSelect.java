...
public class raceSelect extends MainActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button buttDragonborn = (Button) findViewById(R.id.dragonbornButton);  // Button Assignment
        buttDragonborn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view)
            {
                for (int i = 0; i < raceList.size(); i++) {                    // Finds the correct Race in list
                    if (raceList.get(i).name.equals("Dragonborn")) {
                        player.characterRace = raceList.get(i);                // Assigns Race to Button
                    }
                }
                Intent raceToClass = new Intent(raceSelect.this, classSelect.class);    // Passes the character to the next activity
